...
# res$Wilcoxon.p=c(wilcox.test(raw$B4_T5,raw$B4_NGRAM,alternative="two.side",paired=TRUE)$p.value)
# cliff.delta(raw$B4_T5,raw$B4_NGRAM)

# res$Dataset=c(res$Dataset,as.character("../Data/Wilcoxon/abstracted.csv"))
# res$Wilcoxon.p=c(wilcox.test(abstracted$B4_T5,abstracted$B4_NGRAM,alternative="two.side",paired=TRUE)$p.value)
# cliff.delta(abstracted$B4_T5,abstracted$B4_NGRAM)

########### COMMENT when running the Wilcoxon test for the BLEU-4 score ###########
res$Dataset=c(res$Dataset,as.character("../Data/Wilcoxon/raw.csv"))
res$Wilcoxon.p=c(wilcox.test(raw$ROUGE_T5,raw$ROUGE_NGRAM,alternative="two.side",paired=TRUE)$p.value)
cliff.delta(raw$ROUGE_T5,raw$ROUGE_NGRAM)

res$Dataset=c(res$Dataset,as.character("../Data/Wilcoxon/abstracted.csv"))
res$Wilcoxon.p=c(wilcox.test(abstracted$ROUGE_T5,abstracted$ROUGE_NGRAM,alternative="two.side",paired=TRUE)$p.value)
